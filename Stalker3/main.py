...
class Player(Sprite):
    ...
    def drawPlayer(self):
        if self.typePlayer == 'enemy' and self.action == 3 and self.faza_now == 6:
            self.kill()
        if self.typePlayer == "enemy":
            img = pygame.transform.scale(self.image, (int(self.image.get_width() * 0.5), int(self.image.get_height() * 0.5)))
            rect2 = img.get_rect()
            if self.vlevoVpravo == 1:
                screen.blit(self.image, rect2)
            else:
                screen.blit(pygame.transform.flip(self.image, -1, False), rect2)
        else:
            if self.vlevoVpravo == 1:
    ...
    def move_ai(self, go_left, go_right):
        ...
        for block in world.world:
            if block[1].colliderect(self.rect.x + 5, self.rect.y, self.width, self.height):
                couldGoRight = False
                dx = 0
                break
            else:
        ...
        print(couldGoLeft, couldGoRight)
        if self.health > 0:
            if go_left and couldGoLeft:
                dx = -5
                self.menyem = True
                self.vlevoVpravo = -1
            if go_right and couldGoRight:
                dx = 5
                self.menyem = False
                self.vlevoVpravo = 1
